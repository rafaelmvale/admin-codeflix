...
describe('CategoryFakerBuilder Unit Tests', () => {
  ...
  test('should create many categories', () => {
    ...
    categories = faker
      ...
      expect(categories.category_id.id).toBe(category_id.id);
      expect(categories.name).toBe('name test');
      expect(categories.description).toBe('description test');
      expect(categories.is_active).toBe(false);
      expect(categories.created_at).toBe(created_at);
